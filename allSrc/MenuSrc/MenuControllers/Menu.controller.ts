...
const DeleteMenuWithDish = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        console.log(Meal, "mealparam", Dish, "dishparam");

        const DeleteMenudata = await MenuObj.deleteMealDish(Meal, Dish);

        console.log("DeleteMenudata", DeleteMenudata);
        if (!DeleteMenudata) {
            throw new NotImplementedError(`Deletion of Meal is not performed`);
        }
        res.status(200).json(DeleteMenudata);
    }
    catch (err) {
        next(err);
    }
...
const DeleteDishWithIng = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const DeleteDishIngdata = await MenuObj.deleteDishIng(Dish, IngName);
        if (!DeleteDishIngdata) {

            throw new NotImplementedError(`failed to Delete Dish`);
        }
...
export { MenuCreate, DishIngCreate, MenuRead, DishRead, DeleteDishWithIng, DeleteMenuWithDish };